...
public class ViewTop {
    ...
    public void buildData() {
        ...
        topTable.getColumns().clear();

        try {
            CallableStatement cstmt = conn.prepareCall("{? = call lista_podcasturi(?)}");
            cstmt.registerOutParameter(1, OracleTypes.CURSOR);
            System.out.println(genres.getValue().toString());
            cstmt.setString(2, genres.getValue().toString());
            ...
            for (int i = 0; i < rs.getMetaData().getColumnCount(); i++) {
                ...
                topTable.getColumns().addAll(col);
